...
func updateQuantity(serialNo uint) {
	...
	for _, targetItem := range menu {
		...
		if serialNo == targetItem.itemNo {
			...
			//update bill
			subTotalBillExcludetaxes -= float64(oldQuantity) * float64(targetItem.itemPrice) //delete the item price for old quantity
			subTotalBillExcludetaxes += float64(newQuantity) * float64(targetItem.itemPrice) //add the item price for updated quantity
...
func delFromOrder(serialNo uint) {
	...
	for _, targetItem := range menu {
		if serialNo == targetItem.itemNo {
			...
			subTotalBillExcludetaxes -= float64(customerOrder[itemName]) * float64(targetItem.itemPrice)
			//delete item
